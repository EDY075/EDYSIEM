...
type EntityType = 
  | "ip" | "hostname" | "user" | "ioc" | "hash" 
  | "domain" | "alert" | "incident" | "case" | "mitre" 
  | "asset" | "rule";
...
const MOCK_ENTITIES: Record<string, any[]> = {
  ...
  user: [
    ...
    { id: "john.doe", label: "john.doe", description: "Analista SOC", icon: "👤", route: "/users/john.doe" },
  ],
  ioc: [
  ...
  asset: [
    ...
    { id: "asset-002", label: "WKS-001", type: "workstation", criticality: "medium", route: "/assets/WKS-001" },
  ],
  rule: [
...
const MOCK_ENTITIES: Record<string, any[]> = {
  ...
  domain: [
    { id: "evil.com", label: "evil.com", description: "Malicious domain", icon: "🌐", route: "/domains/evil.com" },
    { id: "c2.badguy.com", label: "c2.badguy.com", description: "C2 Domain", icon: "🎯", route: "/domains/c2.badguy.com" },
  ],
  alert: [
    { id: "ALT-001", label: "Brute Force SSH", severity: "high", icon: "🚨", route: "/alerts/ALT-001" },
    { id: "ALT-002", label: "Malware Detection", severity: "critical", icon: "🦠", route: "/alerts/ALT-002" },
...
export function GlobalSearch() {
  ...
  const search = useCallback((q: string): SearchResultItem[] => {
    ...
    for (const [entityType, entities] of Object.entries(MOCK_ENTITIES)) {
      for (const entity of entities) {
        ...
        if (searchable.includes(q)) {
          const isExact = entity.label.toLowerCase() === q;
          results.push({
            id: entity.id,
            type: entityTypeLabels[entityType as keyof typeof MOCK_ENTITIES],
            label: entity.label,
    ...
    const typeOrder = { alert: 0, incident: 1, case: 2, ip: 2, hostname: 3, user: 3, ioc: 4, domain: 4, hash: 4, asset: 5, mitre: 5, rule: 5 };
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setSelectedIndex(i => Math.min(i + 1, getFilteredResults().length - 1));
      ...
      if (results[selectedIndex]) {
        if (results[selectedIndex].route) {
  ...
  const visibleResults = isOpen ? filteredResults : [];

  return (
    <div style={{ position: "relative", width: "100%" }}>
      <input
        ref={inputRef}
        placeholder="Buscar IP, hostname, usuário, IOC, hash, domínio, alerta, incidente, caso, MITRE, ativo, regra...  [/]"
        value={query}
        onChange={(e) => {
          ...
          {filteredResults.length === 0 ? (
            ...
            <>
              ...
              {ENTITY_TYPES.map(({ key, label, icon }) => {
                const typeResults = filteredResults.filter(r => r.type.toLowerCase() === key.toLowerCase());
                if (typeResults.length === 0) return null;
                
                return (
                  <div key={key} style={{ borderTop: `1px solid ${colors.borderSubtle}`, paddingTop: spacing["2"] }}>
                    <div style={{ 
                      display: "flex", 
                      alignItems: "center", 
                      gap: spacing["2"], 
